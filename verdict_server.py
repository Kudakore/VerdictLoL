...
def validate_riot_id(riot_id: str) -> str:
    """Validate Riot ID format (Name#Tag). Raises HTTPException if invalid."""
    if "#" not in riot_id:
        raise HTTPException(400, f"Invalid Riot ID format: '{riot_id}'. Expected 'Name#Tag'.")
    parts = riot_id.split("#", 1)
    if not parts[0].strip() or not parts[1].strip():
        raise HTTPException(400, f"Invalid Riot ID format: '{riot_id}'. Name and tag must be non-empty.")
    return riot_id
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        cache = {"puuid": None, "games": [], "cached_ids": [], "rank_history": [], "last_updated": None}
    state.cache = cache
...
async def api_error_handler(request, exc: APIError):
    ...
    if exc.status_code == 403:
        return JSONResponse(status_code=503, content={"detail": "Riot API key invalid or unauthorized."})
    if exc.status_code == 429:
        return JSONResponse(status_code=429, content={"detail": "Riot API rate limit exceeded. Try again later."})
    return JSONResponse(status_code=502, content={"detail": f"Riot API error: {exc}"})
...
async def health():
    ...
    return {
        ...
        "pipeline_ready": state.own_service.pipeline_ready if state.own_service else False,
    }
...
async def fetch_games(request: FetchRequest):
    state: AppState = app.state
    if not state.config_valid:
        raise HTTPException(503, "Config incomplete. Set VERDICT_API_KEY and player identity.")

    count, force, background = request.count, request.force, request.background

    if background:
        async with state._pipeline_lock:
...
async def worst(champion: Optional[str] = None):
    ...
    result = await run_sync(service.analyze_worst, champion)
    if champion and isinstance(result, dict) and result.get("header", {}).get("total_games") == 0:
        result["warning"] = f"No games found for champion '{champion}'. Check spelling (case-insensitive)."
    return result
...
async def best(champion: Optional[str] = None):
    state: AppState = app.state
    service = require_service(state)
    result = await run_sync(service.analyze_best, champion)
    if champion and isinstance(result, dict) and result.get("header", {}).get("total_games") == 0:
        result["warning"] = f"No games found for champion '{champion}'. Check spelling (case-insensitive)."
    return result
...
@app.get("/api/v1/pool")
async def pool(min_games: int = Query(default=3, ge=1, le=50)):
    state: AppState = app.state
    service = require_service(state)
...
async def matchups(champion: Optional[str] = None):
    state: AppState = app.state
    service = require_service(state)
    result = await run_sync(service.analyze_matchups, champion)
    if champion and isinstance(result, dict):
        header = result.get("header", {})
        if header.get("total_games", 0) == 0:
            result["warning"] = f"No games found for champion '{champion}'. Check spelling (case-insensitive)."
    return result
...
async def recent(queue: Optional[str] = None, count: int = Query(default=20, ge=1, le=100)):
    state: AppState = app.state
    service = require_service(state)
...
async def games_list(champion: Optional[str] = None, count: int = Query(default=5, ge=1, le=50)):
    state: AppState = app.state
    games = require_games(state)
    if champion:
        filtered = [g for g in games if g.champion.lower() == champion.lower()]
        if not filtered and champion:
            return {"warning": f"No games found for champion '{champion}'. Check spelling (case-insensitive).", "games": []}
        games = filtered
    games = games[:count]
    results = []
...
async def select_games(champion: Optional[str] = None, result: Optional[str] = None, page: int = Query(default=0, ge=0, le=1000), page_size: int = Query(default=10, ge=1, le=100)):
    state: AppState = app.state
    cache = state.cache
    if not cache:
        raise HTTPException(404, "No cached games. Run /api/v1/fetch first.")
...
async def scout(riot_id: str, count: int = Query(default=20, ge=1, le=100)):
...
async def compare(riot_id: str, count: int = Query(default=20, ge=1, le=100)):
    validate_riot_id(riot_id)
    state: AppState = app.state
